    ...
    <div class="dialog-form">
      ...
      <div class="dialog-form__body">
        ...
        <div class="dialog-form__group">
          ...
          <input type="file" ref="imgFile" @change="uploadImg" />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    uploadImg() {
      ...
      this.propsChoose.imgName = this.getMaxId + imgFile.name
    ...
    async editAction() {
      // 照片名稱不一樣時，執行
      if (this.beforeImg !== this.propsChoose.imgName) {
        console.log('更新照片')
        // 刪除原本照片
        storageRef.child(`image/${this.beforeImg}`).delete()
        // 更新新的照片
        await storageRef
          .child('image/' + this.propsChoose.imgName)
  ...
  mounted() {
    //刪除照片使用
    if (Object.keys(this.boxDate).length !== 0) {
      this.beforeImg = this.boxDate.imgName
      console.log('修改', this.beforeImg)
    }
  },
}
</script>
